...
class AmqpConnection {
  ...
  consumeByAli(sms, exchange, queue, bindingkey, type, option) {
    if (!sms) return console.error('请传入sms配置,谢谢！');
    let alidayu = new Alidayu(sms.clientOption, sms.smsOption);
    if (!option) option = { durable: false };
    let ch;
    this.createChannel()
      .then((channel) => {
      ...
      .then((ok) => {
        ...
        ch.consume(q, (msg) => {
          doWork(alidayu, msg.content.toString());
        }, { noAck: true })
      })
  }

  consumeByCloud(sms, exchange, queue, bindingkey, type, option) {
    if (!sms) return console.error('请传入sms配置,谢谢！');
    let cloud = new Cloud(sms.config, sms.option);
    if (!option) option = { durable: false };
    let ch;
    this.createChannel()
      .then((channel) => {
      ...
      .then((ok) => {
        ...
        ch.bindQueue(q, exchange, bindingkey);
        ch.consume(q, (msg) => {
          cloud.sendSms(msg.content.toString()).then(() => {
            console.log("发送成功");
          }).catch(err => {
  ...
  consumeBySDK(sms, exchange, queue, bindingkey, type, option) {
    if (!sms) return console.error('请传入sms配置,谢谢！');
    let sdk = new SDK(sms);
    if (!option) option = { durable: false };
    let ch;
    this.createChannel()
      ...
      .then((ok) => {
        ...
        ch.consume(q, (msg) => {
          ...
          sdk.sendBySingle(msg.tel, msg.content).then(() => {
            console.log("发送成功");
          }).catch(err => {
            console.error("err", err);
          })
...
function doWork(alidayu, msg) {
  alidayu.send(JSON.parse(msg)).then((res) => {
